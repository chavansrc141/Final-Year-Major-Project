...
data = sio.loadmat('data4drl/data_pavia_university_drl.mat')['paviaU']
data = data.reshape(data.shape[0]*data.shape[1], data.shape[2]).astype(np.float32)
print(data.shape)
sio.savemat('pavia_university.mat', {'x': data})
x = np.float32(data)
...
# ----------------------- 
# agent.saveWeights('models/qnet_pavia_university_30_bands.h5')
agent.loadWeights('models/qnet_pavia_university_30_bands.h5')
...
sio.savemat('results/drl_30_bands_pavia_university.mat', {'selected_bands': selected_bands})
